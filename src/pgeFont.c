...
pgeFont* pgeFontLoadMem(const void *fonttext,int memsize, unsigned int fontSize, enum pgeFontSizeType fontSizeType, unsigned int textureSize)
{
	FT_Library library;
	FT_Face face;
	FT_Open_Args  args;
    ...
    FT_New_Memory_Face( library,fonttext,memsize,0, &face );
	...
	for(n = 0; n < count; n++)
	{
		FT_Load_Char(face, PGE_FONT_CHARSET[n], FT_LOAD_RENDER);
	...
	{
		FT_Load_Char(face, gi[n].c, FT_LOAD_RENDER);
...
int pgeFontPrintX(pgeFont *font, float x, float y,  const char *text,unsigned int color,int *lastposx)
{
	if(!font)
		return 0;
...
int pgeFontPrint(pgeFont *font, float x, float y,  const char *text,unsigned int color)
{
	if(!font)
		return 0;
